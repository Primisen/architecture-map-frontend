...
@Component({
  selector: 'app-searching-with-filters',
  templateUrl: './searching-with-filters.component.html',
  styleUrls: ['./searching-with-filters.component.css']
})
export class SearchingWithFiltersComponent {
  ...
  getArchitecturalStyles() {
    this.getResource(this.getArchitecturalStylesUrl)
      .subscribe(
        (architecturalStyle: ArchitecturalStyle[]) => { this.architecturalStyles = architecturalStyle.sort((a, b) => a.name.localeCompare(b.name)) }
      );
  }
  ...
  search() {
    ...
    this.httpClient.get(this.searchConstructionUrl, {params}).subscribe(
      answer => this.searchParams.emit(answer)
    )
  }
}
